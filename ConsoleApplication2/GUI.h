...
//------------------------------------------------------------------------------
    ...
    struct Out_box : Widget {
        Out_box(Point xy, int w, int h, const string& s)
            :Widget(xy,w,h,s,0) { }
		string lastString;//out_box will hold onto the last string
		bool remember = true;// set to false to prevent remembering
		bool wasHighlighted = false;
        void put(int);
        void put(const string&);
		void highlight(){ 
			wasHighlighted = true;
			reference_to<Fl_Output>(pw).textfont(FL_HELVETICA_BOLD); 
			Fl::redraw();
		}
		void unhighlight(){ 
			wasHighlighted = false;
			reference_to<Fl_Output>(pw).textfont(FL_HELVETICA);
			Fl::redraw();
		}
		string get(){
			string val = reference_to<Fl_Output>(pw).value();
			return val;
		}
